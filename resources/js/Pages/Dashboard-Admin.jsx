...
import { Menu, Package, ShoppingCart, FileText, Users, LogOut, ChevronDown, AlertTriangle, Bell, Home, Plus } from 'lucide-react';
...
const Dashboard = () => {
    ...
    if (restockModal.item && restockAmount) {
      const updatedItems = stockItems.map(item => {
        if (item.id === restockModal.item.id) {
          return { ...item, stock: item.stock + parseInt(restockAmount) };
        }
        return item;
      });
  ...
  const currentLowStockItems = stockItems.filter(item => item.stock < item.minStock);
            ...
            <div className="bg-white p-4 rounded-lg shadow-sm border border-gray-200">
              <div className="flex justify-between items-start">
                <div>
                  ...
                  <h3 className="text-2xl font-bold text-gray-800">{currentLowStockItems.length}</h3>
                </div>
            ...
            <div className="bg-white p-4 rounded-lg shadow-sm border border-gray-200">
              <div className="flex justify-between items-start">
                <div>
                  ...
                  <h3 className="text-2xl font-bold text-gray-800">{currentExpiringItems.length}</h3>
                </div>
      ...
      {restockModal.open && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white p-6 rounded-lg shadow-xl max-w-md w-full mx-4">
            <h3 className="text-lg font-semibold text-gray-800 mb-4">Restok Obat</h3>
            <div className="mb-4">
              <p className="text-sm text-gray-600 mb-2">Nama Obat:</p>
              <p className="font-medium text-gray-800">{restockModal.item?.name}</p>
            </div>
            <div className="mb-4">
              <p className="text-sm text-gray-600 mb-2">Stok Saat Ini: <span className="font-medium text-red-600">{restockModal.item?.stock}</span></p>
              <p className="text-sm text-gray-600">Minimum Stok: <span className="font-medium">{restockModal.item?.minStock}</span></p>
            </div>
            <div className="mb-6">
              <label className="block text-sm text-gray-600 mb-2">Jumlah Tambahan Stok:</label>
              <input
                type="number"
                value={restockAmount}
